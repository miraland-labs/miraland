...
fn load_release_target(release_dir: &Path) -> Result<String, String> {
    ...
    version_yml.push("miraland-release");
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |miraland-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/miraland-labs/miraland/releases/download/v{}/miraland-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "https://release.miraland.top/{}/miraland-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "https://release.miraland.top/{}/miraland-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn print_update_manifest(update_manifest: &UpdateManifest) {
    let when = Local.timestamp_opt(update_manifest.timestamp_secs as i64, 0).unwrap(); // MI
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/miraland-labs/miraland/releases",
            &[
...
pub fn init_or_update(config_file: &str, is_init: bool, check_only: bool) -> Result<bool, String> {
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("miraland-release").join("version.yml");
    ...
    symlink_dir(
        release_dir.join("miraland-release"),
