...
fn tune_poh_service_priority(uid: u32) {
    ...
    if let Some(pid) = find_pid("miraland-validato", "/proc", uid, |dir| {
        let mut path = dir.path();
        path.push("task");
...
fn main() {
    ...
    if let Err(e) = std::fs::remove_file(miraland_sys_tuner::MIRALAND_SYS_TUNER_PATH) {
        if e.kind() != std::io::ErrorKind::NotFound {
            panic!("Failed to remove stale socket file: {:?}", e)
        }
    }

    let listener = unix_socket::UnixListener::bind(miraland_sys_tuner::MIRALAND_SYS_TUNER_PATH)
    ...
    if let Some(user) = users::get_user_by_name(&user) {
        ...
        nix::unistd::chown(
            miraland_sys_tuner::MIRALAND_SYS_TUNER_PATH,
