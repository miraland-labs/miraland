...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about("Deploy a non-upgradeable program. Use `miraland program deploy` instead to deploy upgradeable programs")
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`miraland-keygen recover` and the following {}-word seed phrase:",
        words
    );
    ...
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `miraland program close`.\n{}",
        divider
    );
}
