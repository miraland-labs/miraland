...
fn derive_abi_sample_enum_type(input: ItemEnum) -> TokenStream {
    ...
    let result = quote! {
        ...
        #( #attrs )*
        impl #impl_generics ::miraland_frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
            fn example() -> Self {
                ::log::info!(
                    "AbiExample for enum: {}",
...
fn do_derive_abi_enum_visitor(input: ItemEnum) -> TokenStream {
    ...
    (quote! {
        impl #impl_generics ::miraland_frozen_abi::abi_example::AbiEnumVisitor for #type_name #ty_generics #where_clause {
            fn visit_for_abi(&self, digester: &mut ::miraland_frozen_abi::abi_digester::AbiDigester) -> ::miraland_frozen_abi::abi_digester::DigestResult {
...
fn quote_for_test(
    ...
    quote! {
        ...
        mod #test_mod_ident {
            use super::*;
            use ::miraland_frozen_abi::abi_example::{AbiExample, AbiEnumVisitor};

            #[test]
            fn test_abi_digest() {
                ::miraland_logger::setup();
                let mut digester = ::miraland_frozen_abi::abi_digester::AbiDigester::create();
                ...
                if ::std::env::var("SOLANA_ABI_BULK_UPDATE").is_ok() {
                    ...
                    if let Ok(dir) = ::std::env::var("MIRALAND_ABI_DUMP_DIR") {
                        ...
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with MIRALAND_ABI_DUMP_DIR!");
                    }
                }
            }
        }
