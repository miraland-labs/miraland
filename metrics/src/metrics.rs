...
fn get_metrics_config() -> Result<MetricsConfig, String> {
    ...
    let config_var = env::var("MIRALAND_METRICS_CONFIG")
        .map_err(|err| format!("MIRALAND_METRICS_CONFIG: {}", err))?;

    for pair in config_var.split(',') {
        let nv: Vec<_> = pair.split('=').collect();
        if nv.len() != 2 {
            return Err(format!("MIRALAND_METRICS_CONFIG is invalid: '{}'", pair));
        }
        ...
        match nv[0] {
            ...
            "u" => config.username = v,
            "p" => config.password = v,
            _ => return Err(format!("MIRALAND_METRICS_CONFIG is invalid: '{}'", pair)),
        }
    }

    if !config.complete() {
        return Err("MIRALAND_METRICS_CONFIG is incomplete".to_string());
    }
    Ok(config)
}
