...
use {
    ...
    miraland_logger,
    miraland_svm::transaction_results::DurableNonceFee,
    rand::Rng,
    ...
    miraland_stake_program::stake_state::{self, StakeStateV2},
...
fn create_genesis_config_no_tx_fee_no_rent(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let genesis_config_info = miraland_runtime::genesis_utils::create_genesis_config(lamports);
    (
...
fn create_genesis_config_no_tx_fee(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let (mut genesis_config, mint_keypair) =
        miraland_sdk::genesis_config::create_genesis_config(lamports);
...
fn test_rent_exempt_executable_account() {
    ...
    let mut account = AccountSharedData::new(account_balance, 0, &miraland_sdk::pubkey::new_rand());
...
fn test_rent_eager_collect_rent_in_partition(should_collect_rent: bool) {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != miraland_sdk::feature_set::skip_rent_rewrites::id()
            && (!should_collect_rent
                || feature_id != miraland_sdk::feature_set::disable_rent_fees_collection::id())
        {
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let stake_id1 = miraland_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = miraland_sdk::pubkey::new_rand();
...
fn test_purge_empty_accounts() {
    ...
    for pass in 0..3 {
        miraland_logger::setup();
        let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(mln_to_lamports(1.));
...
fn test_debits_before_credits() {
    let (genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(mln_to_lamports(2.));
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    let keypair = Keypair::new();
...
fn test_bank_parent_account_spend() {
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(mln_to_lamports(1.0));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_hash_internal_state() {
    let (genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(mln_to_lamports(1.));
...
fn test_bank_squash() {
    miraland_logger::setup();
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(mln_to_lamports(2.));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = miraland_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_add_duplicate_static_program() {
    ...
    bank.add_mockup_builtin(miraland_vote_program::id(), MockBuiltin::vm);
    let bank = bank_forks
        .write()
        .unwrap()
        .insert(bank)
    ...
    let vote_loader_account = bank.get_account(&miraland_vote_program::id()).unwrap();
    let new_vote_loader_account = bank.get_account(&miraland_vote_program::id()).unwrap();
...
fn test_account_ids_after_program_ids() {
    ...
    let instruction = Instruction::new_with_bincode(miraland_vote_program::id(), &10, account_metas);
    let mut tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    let account = bank.get_account(&miraland_vote_program::id()).unwrap();
...
fn test_program_id_as_payer() {
    ...
    tx.message.account_keys.push(miraland_vote_program::id());
...
fn test_bank_hash_consistency() {
    ...
    activate_feature(
        &mut genesis_config,
        miraland_sdk::feature_set::set_exempt_rent_epoch_max::id(),
    );
...
fn test_bank_load_program() {
    ...
    let programdata_key = miraland_sdk::pubkey::new_rand();
...
fn test_bpf_loader_upgradeable_deploy_with_max_len() {
    ...
    let payer_base_balance = LAMPORTS_PER_MLN;
    let deploy_fees = {
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &miraland_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
...
fn test_get_largest_accounts() {
    ...
    // Initialize accounts; all have larger MLN balances than current Bank built-ins
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        miraland_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_accounts_data_size_with_good_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_MLN);
    let bank = Bank::new_for_tests(&genesis_config);
    let bank = bank.wrap_with_bank_forks_for_tests().0;
...
fn test_accounts_data_size_with_bad_transaction() {
    const ACCOUNT_SIZE: u64 = MAX_PERMITTED_DATA_LENGTH;
    let (genesis_config, _mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_MLN);
    let bank = Bank::new_for_tests(&genesis_config);
    let bank = bank.wrap_with_bank_forks_for_tests().0;
    let transaction = system_transaction::create_account(
        &Keypair::new(),
...
fn test_accounts_data_size_and_rent_collection(should_collect_rent: bool) {
    ...
    if should_collect_rent {
        genesis_config
            .accounts
            .remove(&miraland_sdk::feature_set::disable_rent_fees_collection::id());
    }
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_MLN);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            miraland_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_is_in_slot_hashes_history() {
    use miraland_sdk::slot_hashes::MAX_ENTRIES;
...
fn test_rewards_point_calculation_empty() {
    ...
    let (genesis_config, _mint_keypair) = create_genesis_config(mln_to_lamports(1.0));
    let bank = Bank::new_for_tests(&genesis_config);
...
fn test_force_reward_interval_end() {
    let (genesis_config, _mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_MLN);
    let mut bank = Bank::new_for_tests(&genesis_config);
...
fn test_get_stake_rewards_partition_range_panic() {
    let (mut genesis_config, _mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_MLN);
...
fn test_program_execution_restricted_for_stake_account_in_reward_period() {
    use miraland_sdk::transaction::TransactionError::ProgramExecutionTemporarilyRestricted;
...
fn test_system_instruction_unsigned_transaction() {
    let (genesis_config, alice_keypair) = create_genesis_config_no_tx_fee(mln_to_lamports(1.0));
    let alice_pubkey = alice_keypair.pubkey();
...
fn test_failed_simulation_compute_units() {
    let (genesis_config, mint_keypair) = create_genesis_config(LAMPORTS_PER_MLN);
    let program_id = Pubkey::new_unique();
    let bank =
        Bank::new_with_mockup_builtin_for_tests(&genesis_config, program_id, MockBuiltin::vm).0;
