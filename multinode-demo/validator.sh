...
airdrops_enabled=1
node_mln=500 # 500 MLN: number of MLN to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-mln MLN            - Number of MLN this node has been funded from the genesis config (default: $node_mln)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-mln ]]; then
      node_mln="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # miraland-validator options
...
if [[ "$MIRALAND_GPU_MISSING" -eq 1 ]]; then
...
if [[ -n $REQUIRE_LEDGER_DIR ]]; then
  ...
  MIRALAND_CONFIG_DIR="$ledger_dir"
fi
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$MIRALAND_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $MIRALAND_CUDA ]]; then
  program=$miraland_validator_cuda
else
  program=$miraland_validator
fi
...
wallet() {
  (
    set -x
    $miraland_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
setup_validator_accounts() {
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      echo "Adding $node_mln to validator identity account:"
      (
        set -x
        $miraland_cli \
          --keypair "$MIRALAND_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_mln"
      ) || return $?
    fi
...
rpc_url=$($miraland_gossip --allow-private-addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $miraland_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $miraland_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $miraland_keygen new --no-passphrase -so "$authorized_withdrawer"

setup_validator_accounts "$node_mln"
