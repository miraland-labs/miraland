...
rm -rf "$MIRALAND_CONFIG_DIR"/latest-testnet-snapshot
mkdir -p "$MIRALAND_CONFIG_DIR"/latest-testnet-snapshot
(
  cd "$MIRALAND_CONFIG_DIR"/latest-testnet-snapshot || exit 1
  set -x
  wget http://api.testnet.miraland.top/genesis.tar.bz2
  wget --trust-server-names http://testnet.miraland.top/snapshot.tar.bz2
)

snapshot=$(ls "$MIRALAND_CONFIG_DIR"/latest-testnet-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$miraland_ledger_tool create-snapshot \
  --ledger "$MIRALAND_CONFIG_DIR"/latest-testnet-snapshot \
