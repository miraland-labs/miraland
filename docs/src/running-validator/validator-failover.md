...
### Primary Validator
The following additional `miraland-validator` parameters are required to enable
...
following `miraland-validator` command-line argument changes:
...
```bash
$ miraland-validator wait-for-restart-window --identity validator-keypair.json \
  && miraland-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `miraland-validator set-identity
