...
```bash
sudo $(command -v miraland-sys-tuner) --user $(whoami) > sys-tuner.log 2>&1 &
```
...
/home/sol/miraland-validator.log:
...
[solarti-token-specific requests](developing/clients/jsonrpc-api.md#gettokenaccountsbydelegate) --
...
- `solarti-token-mint`: each Solarti token account indexed by its token Mint; used by [getTokenAccountsByDelegate](developing/clients/jsonrpc-api.md#gettokenaccountsbydelegate), and [getTokenLargestAccounts](developing/clients/jsonrpc-api.md#gettokenlargestaccounts)
- `solarti-token-owner`: each Solarti token account indexed by the token-owner address; used by [getTokenAccountsByOwner](developing/clients/jsonrpc-api.md#gettokenaccountsbyowner), and [`getProgramAccounts`](developing/clients/jsonrpc-api.md#getprogramaccounts) requests that include an solarti-token-owner filter.
