...
Large system clock drift can prevent a node from properly participating in Miraland's [gossip protocol](../validator/gossip.md).  Ensure that your system clock is accurate.  To check the current system clock, use:
...
You can generate a custom vanity keypair using miraland-keygen. For instance:

```bash
miraland-keygen grind --starts-with e1v1s:1
```
...
```bash
miraland-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
argument to `miraland-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
> `miraland-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `miraland-validator --dynamic-port-range 11000-11020 ...` will restrict
...
here](https://github.com/miraland-labs/miraland/blob/aa72aa87790277619d12c27f1ebc864d23739557/core/src/ledger_cleanup_service.rs#L26-L37).
...
Assuming you have a user called `mln` on your machine, create the file `/etc/systemd/system/mln.service` with
the following:
...
Environment="PATH=/bin:/usr/bin:/home/mln/.local/share/miraland/install/active_release/bin"
ExecStart=/home/mln/bin/validator.sh

[Install]
WantedBy=multi-user.target
```

Now create `/home/mln/bin/validator.sh` to include the desired
`miraland-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec miraland-validator ..."). This is
important because without it, logrotate will end up killing the validator
every time the logs are rotated.

Ensure that running `/home/mln/bin/validator.sh` manually starts
the validator as expected. Don't forget to mark it executable with `chmod +x /home/mln/bin/validator.sh`
...
The validator log file, as specified by `--log ~/miraland-validator.log`, can get
...
launch the process with `exec` (`exec miraland-validator ...`) when using logrotate.
...
running as a systemd service called `mln.service` and writes a log file at
/home/mln/miraland-validator.log:

```bash
# Setup log rotation

cat > logrotate.mln <<EOF
/home/mln/miraland-validator.log {
  rotate 7
  daily
  missingok
  postrotate
    systemctl kill -s USR1 mln.service
  endscript
}
EOF
sudo cp logrotate.mln /etc/logrotate.d/mln
