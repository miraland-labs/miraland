...
- Collecting signatures from geographically disparate signers in a
  [multi-signature scheme](https://spl.miraland.top/token#multisig-usage)
- Signing transactions using an [airgapped](<https://en.wikipedia.org/wiki/Air_gap_(networking)>)
  signing device

This document describes using Miraland's CLI to separately sign and submit a
transaction.
...
- [`create-stake-account`](cli/usage.md#miraland-create-stake-account)
- [`create-stake-account-checked`](cli/usage.md#miraland-create-stake-account-checked)
- [`deactivate-stake`](cli/usage.md#miraland-deactivate-stake)
- [`delegate-stake`](cli/usage.md#miraland-delegate-stake)
- [`split-stake`](cli/usage.md#miraland-split-stake)
- [`stake-authorize`](cli/usage.md#miraland-stake-authorize)
- [`stake-authorize-checked`](cli/usage.md#miraland-stake-authorize-checked)
- [`stake-set-lockup`](cli/usage.md#miraland-stake-set-lockup)
- [`stake-set-lockup-checked`](cli/usage.md#miraland-stake-set-lockup-checked)
- [`transfer`](cli/usage.md#miraland-transfer)
- [`withdraw-stake`](cli/usage.md#miraland-withdraw-stake)

- [`create-vote-account`](cli/usage.md#miraland-create-vote-account)
- [`vote-authorize-voter`](cli/usage.md#miraland-vote-authorize-voter)
- [`vote-authorize-voter-checked`](cli/usage.md#miraland-vote-authorize-voter-checked)
- [`vote-authorize-withdrawer`](cli/usage.md#miraland-vote-authorize-withdrawer)
...
- [`withdraw-from-vote-account`](cli/usage.md#miraland-withdraw-from-vote-account)
...
```bash
miraland@offline$ miraland transfer --sign-only --blockhash 5Tx8F3jgSHx21CbtjwmdaKPLM5tWmreWAnPrbqHomSJF \
...
```bash
miraland@online$ miraland transfer --blockhash 5Tx8F3jgSHx21CbtjwmdaKPLM5tWmreWAnPrbqHomSJF \
...
```text
miraland@offline1$ miraland transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
...
```text
miraland@offline2$ miraland transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
    --blockhash 7ALDjLv56a8f6sH6upAZALQKkXyjAwwENH9GomyM8Dbc \
    --sign-only \
    --keypair from.json \
...
```text
miraland@online$ miraland transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
    --blockhash 7ALDjLv56a8f6sH6upAZALQKkXyjAwwENH9GomyM8Dbc \
    --from 674RgFMgdqdRoVtMqSBg7mHFbrrNm1h1r721H1ZMquHL \
...
Typically a Miraland transaction must be signed and accepted by the network within
