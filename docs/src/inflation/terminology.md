...
The rate of return (aka _interest_) earned on MLN staked on the network. It is often quoted as an annualized rate (e.g. "the network _staking yield_ is currently $10\%$ per year").
...
- $100\%$ of inflationary issuances are to be distributed to staked token-holders in proportion to their staked MLN and to validators who charge a commission on the rewards earned by their delegated MLN.
...
&\left( 1 - \text{Validator Fee} \right) \times \left( \frac{1}{\%~\text{MLN Staked}} \right) \\
\text{where:}\\
