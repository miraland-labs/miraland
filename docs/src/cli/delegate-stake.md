...
[Staking and Inflation FAQ](https://miraland.top/staking).

------

After you have [received MLN](transfer-tokens.md), you might consider putting
it to use by delegating _stake_ to a validator. Stake is what we call tokens
in a _stake account_. Miraland weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. Miraland then generates
new MLN periodically to reward stakers and validators. You earn more rewards
...
```bash
miraland create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `miraland stake-account` command:

```bash
miraland stake-account <STAKE_ACCOUNT_ADDRESS>
```

The output will look similar to this:

```text
Total Stake: 5000 MLN
...
`miraland stake-authorize` command. For example, to set a new stake authority,
run:

```bash
miraland stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
miraland create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `miraland create-address-with-seed`:

```bash
miraland create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`miraland create-stake-account`.
...
accounts with the `miraland validators` command:
...
address in `miraland delegate-stake`:

```bash
miraland delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `miraland stake-account` to observe the changes
...
Delegated Stake: 4999.99771712 MLN
...
Once delegated, you can undelegate stake with the `miraland deactivate-stake`
command:

```bash
miraland deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `miraland withdraw-stake` command:

```bash
miraland withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `miraland split-stake` command:

```bash
miraland split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
