...
Developers can write and deploy their own programs to the Miraland blockchain.
...
Miraland on-chain programs are compiled via the [LLVM compiler
...
Because Miraland uses the LLVM compiler infrastructure, a program may be written
in any programming language that can target the LLVM's BPF backend. Miraland
...
The virtual address memory map used by Miraland BPF programs is fixed and laid out
as follows
...
operations:
https://github.com/miraland-labs/miraland-program-library/tree/master/libraries/math
