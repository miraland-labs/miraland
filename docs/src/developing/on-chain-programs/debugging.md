...
`export RUST_LOG=miraland_runtime::system_instruction_processor=trace,miraland_runtime::message_processor=info,miraland_bpf_loader=debug,solana_rbpf=debug`
...
Now, connect to the gdb server that `miraland-ledger-tool` implements, and
