...
The Miraland validator community holds regular educational workshops. You can watch past workshops through the [miraland validator educational workshops playlist](https://www.youtube.com/watch?v=86zySQ5vGW8&list=PLilwLeBwGuK6jKrmn7KOkxRxS9tvbRa5p).

## Help with the validator command line

From within the Miraland CLI, you can execute the `miraland-validator` command with the `--help` flag to get a better understanding of the flags and sub commands available.

```
miraland-validator --help
```
...
It is a best practice to always build your Miraland binaries from source. If you build from source, you are certain that the code you are building has not been tampered with before the binary was created. You may also be able to optimize your `miraland-validator` binary to your specific hardware.

If you build from source on the validator machine (or a machine with the same CPU), you can target your specific architecture using the `-march` flag. Refer to the Miraland docs for [instructions on building from source](../../cli/install-miraland-cli-tools.md#build-from-source).

### miraland-install

If you are not comfortable building from source, or you need to quickly install a new version to test something out, you could instead try using the `miraland-install` command.

Assuming you want to install Miraland version `1.18.0`, you would execute the following:

```
miraland-install init 1.18.0
```

This command downloads the executable for `1.18.0` and installs it into a `.local` directory. You can also look at `miraland-install --help` for more options.

> **Note** this command only works if you already have the miraland cli installed. If you do not have the cli installed, refer to [install miraland cli tools](../../cli/install-miraland-cli-tools.md)

### Restart

For all install methods, the validator process will need to be restarted before the newly installed version is in use.  Use `miraland-validator exit` to restart your validator process.
...
To avoid downloading a snapshot on restart, add the following flag to the `miraland-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `miraland-validator` command, make sure that you run `miraland catchup <pubkey>` after your validator starts to make sure that the validator is catching up in a reasonable time. After some time (potentially a few hours), if it appears that your validator continues to fall behind, then you may have to download a new snapshot.
...
It is important that you do not accidentally run out of funds in your identity account, as your node will stop voting. It is also important to note that this account keypair is the most vulnerable of the three keypairs in a vote account because the keypair for the identity account is stored on your validator when running the `miraland-validator` software. How much MLN you should store there is up to you. As a best practice, make sure to check the account regularly and refill or deduct from it as needed. To check the account balance do:

```
miraland balance validator-keypair.json
```

> **Note** `miraland-watchtower` can monitor for a minimum validator identity balance.  See [monitoring best practices](./monitoring.md) for details.
