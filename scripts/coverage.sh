...
if [[ -z $1 ]]; then
  packages=(--lib --all --exclude miraland-local-cluster)
else
  packages=("$@")
fi
...
if RUST_LOG=solana=trace,miraland=trace _ ci/intercept.sh "$cargo" nightly test --jobs "$JOBS" --target-dir target/cov "${packages[@]}" -- --nocapture; then
  test_status=0
else
  test_status=$?
