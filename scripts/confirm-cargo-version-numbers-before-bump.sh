...
Checks that the tag matches the branch (unless branch is main) and the Cargo.toml versions match the tag.
EOF
  exit 0
}
...
# The tag is expected to be the branch name plus a patch number (unless branch is main). eg:
#   tag:    v1.2.3
#   branch: v1.2
 if [[ "$tag" != "$branch"* && $branch != "main" ]]; then
    >&2 echo "Tag must start with the branch name (unless branch is main). Tag: $tag   Branch: $branch"
    exit 1
fi
