...
MIRALAND_ROOT="$("${readlink_cmd}" -f "${here}/..")"
cargo="${MIRALAND_ROOT}/cargo"
...
(
  ...
  if [[ -z "$validatorOnly" ]]; then
    # shellcheck source=scripts/solarti-token-cli-version.sh
    source "$MIRALAND_ROOT"/scripts/solarti-token-cli-version.sh
    ...
    "$cargo" $maybeRustVersion \
      ...
      install --locked solarti-token-cli --root "$installDir" $maybeSolartiTokenCliVersionArg
  fi
)
