...
export RUST_LOG=${RUST_LOG:-solana=info,miraland=info,miraland_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
...
if [[ -e "$ledgerDir"/genesis.bin || -e "$ledgerDir"/genesis.tar.bz2 ]]; then
  ...
  miraland-genesis \
    ...
    --ledger "$ledgerDir" \
    --cluster-type "$MIRALAND_RUN_SH_CLUSTER_TYPE" \
    $SPL_GENESIS_ARGS \
    $MIRALAND_RUN_SH_GENESIS_ARGS
fi
