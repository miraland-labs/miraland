...
//! standard signing sources, from the command line, as in the Miraland CLI.
//!
...
//! sources supported by the Miraland CLI. Many other functions here are
...
impl DefaultSigner {
    ...
    /// use miraland_clap_v3_utils::keypair::DefaultSigner;
            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"miraland-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    /// ```no_run
    /// use clap::{Arg, Command};
    /// use miraland_clap_v3_utils::keypair::{SignerFromPathConfig, DefaultSigner};
    /// use miraland_clap_v3_utils::offline::OfflineArgs;
    ///
    /// let clap_app = Command::new("my-program")
    ///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::signer_from_path;
/// use miraland_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::{signer_from_path_with_config, SignerFromPathConfig};
/// use miraland_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{path}\". Run \"miraland-keygen new\" to create a keypair file: {e}"),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::pubkey_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"miraland-keygen new\" to create a keypair file: {e}"
                ),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::keypair_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
...
/// ```no_run`
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::elgamal_keypair_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run`
/// use clap::{Arg, Command};
/// use miraland_clap_v3_utils::keypair::ae_key_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("ae-key")
