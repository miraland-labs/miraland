...
//!
//! This crate implements 3 miraland-runtime traits (`InstalledScheduler`, `UninstalledScheduler` and
...
//! `miraland-ledger`'s helper function called `execute_batch()`.
...
use {
    ...
    log::*,
    miraland_ledger::blockstore_processor::{
...
// SchedulerPool must be accessed as a dyn trait from miraland-runtime, because SchedulerPool
// contains some internal fields, whose types aren't available in miraland-runtime (currently
    ...
    fn start_threads(&mut self, context: &SchedulingContext) {
        ...
        self.handler_threads = (0..self.pool.handler_count)
            .map({
                |thx| {
                    thread::Builder::new()
                        .name(format!("mlnScHandler{:02}", thx))
