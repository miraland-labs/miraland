...
if [[ $cloudProvider = ec2 ]]; then
  ...
  sshPrivateKey="$HOME/.ssh/miraland-net-id_$prefix"
else
    ...
    if [[ ! -r $sshPrivateKey ]]; then
      ...
      (
        ...
        for i in $(seq 1 60); do
          set -x
          cloud_FetchFile "$nodeName" "$nodeIp" /miraland-scratch/id_ecdsa "$sshPrivateKey" "$nodeZone" &&
            cloud_FetchFile "$nodeName" "$nodeIp" /miraland-scratch/id_ecdsa.pub "$sshPrivateKey.pub" "$nodeZone" &&
              break
          set +x
  ...
  recordInstanceIp() {
    ...
    (
      ...
      for i in $(seq 1 60); do
        (
          set -x
          timeout --preserve-status --foreground 20s ssh "${sshOptions[@]}" "$publicIp" "ls -l /miraland-scratch/.instance-startup-complete"
        )
        ret=$?
        if [[ $ret -eq 0 ]]; then
...
if [[ -f /miraland-scratch/.instance-startup-complete ]]; then
  echo reboot
  $(
    cd "$here"/scripts/
    if "$enableGpu"; then
  ...
  if [[ -x ~miraland/miraland/on-reboot ]]; then
    sudo -u miraland ~miraland/miraland/on-reboot
  fi
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  To block until setup is complete, run:
    $ until [[ -f /miraland-scratch/.instance-startup-complete ]]; do sleep 1; done

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$(creationInfo)
EOM

# Place the generated private key at /miraland-scratch/id_ecdsa so it's retrievable by anybody
# who is able to log into this machine
mkdir -m 0777 /miraland-scratch
cat > /miraland-scratch/id_ecdsa <<EOK
$(cat "$sshPrivateKey")
EOK
cat > /miraland-scratch/id_ecdsa.pub <<EOK
$(cat "$sshPrivateKey.pub")
EOK
chmod 444 /miraland-scratch/id_ecdsa
...
$(
  ...
  cat \
    ...
    miraland-user-authorized_keys.sh \
    miraland-user-authorized_keys.sh \
    add-testnet-miraland-user-authorized_keys.sh \
    install-ag.sh \
    install-certbot.sh \
    install-earlyoom.sh \
    install-iftop.sh \
...
cat >/miraland-scratch/gce-self-destruct.sh <<'EOS'
...
cat >/miraland-scratch/gce-self-destruct-ps1.sh <<'EOS'
#!/usr/bin/env bash
source "$(dirname "$0")/gce-self-destruct.sh"
gce_self_destruct_ps1
EOS
chmod +x /miraland-scratch/gce-self-destruct-ps1.sh

# Append MOTD and PS1 replacement to .profile
cat >>~miraland/.profile <<'EOS'

# Print self-destruct countdown on login
source "/miraland-scratch/gce-self-destruct.sh"
gce_self_destruct_motd

# Add self-destruct countdown to terminal prompt
export PS1='\[\e]0;\u@\h: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]$(/miraland-scratch/gce-self-destruct-ps1.sh):\[\033[01;34m\]\w\[\033[00m\]\$ '
EOS
EOSD
    cat <<EOSD
...
tmpfsMountPoint=/mnt/miraland-accounts
