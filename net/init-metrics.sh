...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and MIRALAND_METRICS_CONFIG is
...
host="https://metrics.miraland.top:8086"
...
if $useEnv; then
  [[ -n $MIRALAND_METRICS_CONFIG ]] ||
    usage "MIRALAND_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  MIRALAND_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export MIRALAND_METRICS_CONFIG=\"$MIRALAND_METRICS_CONFIG\"" >> "$configFile"
