...
if [[ ! -f "${MIRALAND_LOCK_FILE}" ]]; then
  exec 9>>"${MIRALAND_LOCK_FILE}"
  flock -x -n 9 || ( echo "Failed to acquire lock!" 1>&2 && exit 1 )
  MIRALAND_USER="${MIRALAND_USER:?"MIRALAND_USER undefined"}"
  {
    echo "export MIRALAND_LOCK_USER=${MIRALAND_USER}"
    echo "export MIRALAND_LOCK_INSTANCENAME=${INSTANCE_NAME}"
    echo "export PREEMPTIBLE=${PREEMPTIBLE}"
    echo "[[ -v SSH_TTY && -f \"${HOME}/.miraland-motd\" ]] && cat \"${HOME}/.miraland-motd\" 1>&2"
  } >&9
  exec 9>&-
