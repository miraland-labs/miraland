...
if [[ -f "${MIRALAND_LOCK_FILE}" ]]; then
  ...
  . "${MIRALAND_LOCK_FILE}"
  if [[ "${MIRALAND_LOCK_USER}" = "${MIRALAND_USER}"  || -n "${FORCE_DELETE}" ]]; then
    ...
    git clean -qxdff
    rm -f /miraland-scratch/* /miraland-scratch/.[^.]*
...
EOAK
    ...
    echo "Invalid user: expected \"${MIRALAND_LOCK_USER}\" got \"${MIRALAND_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
