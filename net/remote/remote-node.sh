...
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$MIRALAND_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          miraland-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            miraland-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(miraland-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        miraland-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(miraland-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(miraland-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(miraland-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        miraland-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      miraland-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(miraland-ledger-tool -l config/bootstrap-validator bank-hash --halt-at-slot 0)
        shredVersion="$(cat "$MIRALAND_CONFIG_DIR"/shred-version)"
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/miraland-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/miraland/config/blockstreamer-identity.json "$MIRALAND_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/miraland/config/validator-identity-"$nodeIndex".json "$MIRALAND_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/miraland/config/validator-stake-"$nodeIndex".json "$MIRALAND_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/miraland/config/validator-vote-"$nodeIndex".json "$MIRALAND_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/miraland/config/shred-version "$MIRALAND_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/miraland/config/bank-hash "$MIRALAND_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/miraland/config/faucet.json "$MIRALAND_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$MIRALAND_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/miraland-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$MIRALAND_CONFIG_DIR"/validator-identity.json ]]; then
      miraland-keygen new --no-passphrase -so "$MIRALAND_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$MIRALAND_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$MIRALAND_CONFIG_DIR"/vote-account.json ]]; then
      miraland-keygen new --no-passphrase -so "$MIRALAND_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$MIRALAND_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$MIRALAND_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$MIRALAND_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/miraland/config/faucet.json "$MIRALAND_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      miraland --url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$MIRALAND_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$MIRALAND_CONFIG_DIR"/stake-account.json \
                                         --force \
