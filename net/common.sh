...
MIRALAND_ROOT="$netDir"/..
# shellcheck source=scripts/configure-metrics.sh
source "$MIRALAND_ROOT"/scripts/configure-metrics.sh
...
MIRALAND_CONFIG_DIR=$MIRALAND_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$MIRALAND_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$MIRALAND_CONFIG_DIR"
    fi
  )
}
