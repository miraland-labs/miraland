...
//!
//! Includes definitions and helpers for Miraland derivation paths.
//! The standard Miraland BIP-44 derivation path prefix is
//!
//! > `m/44'/2002'`
//!
//! with 2002 being the Miraland coin type.
...
impl DerivationPath {
    ...
    pub fn from_key_str(path: &str) -> Result<Self, DerivationPathError> {
        Self::from_key_str_with_coin(path, Miraland)
    }
    ...
    pub fn new_bip44(account: Option<u32>, change: Option<u32>) -> Self {
        Self::new_bip44_with_coin(Miraland, account, change)
    }
    ...
    fn from_uri(
        ...
        if let Some(query) = uri.query() {
            ...
            if let Some(key) = key {
                ...
                // generates a Miraland bip44 DerivationPath
...
// MI: Solarti, Qthor, Mira, Miraland
struct Solarti;

impl Bip44 for Solarti {
    const COIN: u32 = 1985;
}

struct Qthor;

impl Bip44 for Qthor {
    const COIN: u32 = 1986;
}
...
mod tests {
    ...
    fn test_derivation_path_debug() {
        ...
        assert_eq!(format!("{path:?}"), "m/44'/2002'".to_string());

        let path = DerivationPath::new_bip44(Some(1), None);
        assert_eq!(format!("{path:?}"), "m/44'/2002'/1'".to_string());

        let path = DerivationPath::new_bip44(Some(1), Some(2));
        assert_eq!(format!("{path:?}"), "m/44'/2002'/1'/2'".to_string());
    }
}
