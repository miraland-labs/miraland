...
//!   active then the `miraland_client` crate logs at the "debug" level any logs
//!   for transactions that failed during simulation. If using [`env_logger`]
//!   these logs can be activated by setting `RUST_LOG=miraland_client=debug`.
...
//! [`RpcClient::get_transaction`]: https://docs.rs/miraland-client/latest/miraland_client/rpc_client/struct.RpcClient.html#method.get_transaction
//!
