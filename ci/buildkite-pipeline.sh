...
all_test_steps() {
  command_step checks ". ci/rust-version.sh; ci/docker-run.sh \$\$rust_nightly_docker_image ci/test-checks.sh" 40
  wait_step
  ...
  # Full test suite
  command_step stable ". ci/rust-version.sh; ci/docker-run.sh \$\$rust_stable_docker_image ci/test-stable.sh" 180
  ...
  if affects \
             ...
             ^ci/test-docs.sh \
      ; then
    command_step doctest "ci/test-docs.sh" 90
  else
...
EOF
  ...
  if affects \
             ...
             ^ci/test-bench.sh \
      ; then
    command_step bench "ci/test-bench.sh" 120
  else
