...
all_test_steps() {
  command_step checks1 "ci/docker-run-default-image.sh ci/test-checks.sh" 90 check
  command_step checks2 "ci/docker-run-default-image.sh ci/test-dev-context-only-utils.sh check-bins" 90 check
  command_step checks3 "ci/docker-run-default-image.sh ci/test-dev-context-only-utils.sh check-all-targets" 90 check
  wait_step
  ...
  if affects \
             ...
             ^ci/test-docs.sh \
      ; then
    command_step doctest "ci/docker-run-default-image.sh ci/test-docs.sh" 60
  else
...
EOF
  ...
  if affects \
             ...
             ^sdk/ \
      ; then
    command_step wasm "ci/docker-run-default-image.sh ci/test-wasm.sh" 60
  else
    annotate --style info \
  ...
  if affects \
             ...
             ^scripts/coverage.sh \
      ; then
    command_step coverage "ci/docker-run-default-image.sh ci/test-coverage.sh" 120
  else
...
pull_or_push_steps() {
  ...
  if affects .toml$ && affects .lock$ && ! affects_other_than .toml$ .lock$; then
    ...
    if [ -z "$diff_other_than_version_bump" ]; then
      ...
      command_step checks "ci/docker-run-default-image.sh ci/test-checks.sh" 90
      exit 0
    fi
  fi
