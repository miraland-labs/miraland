...
cat >/etc/apt/apt.conf.d/99-miraland <<'EOF'
...
"$MIRALAND_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$MIRALAND_ROOT"/net/scripts/install-certbot.sh
...
"$MIRALAND_ROOT"/net/scripts/install-earlyoom.sh
"$MIRALAND_ROOT"/net/scripts/install-nodejs.sh
"$MIRALAND_ROOT"/net/scripts/localtime.sh
"$MIRALAND_ROOT"/net/scripts/install-redis.sh
"$MIRALAND_ROOT"/net/scripts/install-rsync.sh
"$MIRALAND_ROOT"/net/scripts/install-libssl-compatability.sh
