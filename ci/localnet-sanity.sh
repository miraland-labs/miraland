...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $miraland_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $miraland_validator --ledger "$MIRALAND_CONFIG_DIR"/bootstrap-validator exit --force || true
  $miraland_validator --ledger "$MIRALAND_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $miraland_ledger_tool --ledger "$MIRALAND_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $miraland_keygen new --no-passphrase -fso $client_keypair || exit $?
    $miraland_gossip --allow-private-addr spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
