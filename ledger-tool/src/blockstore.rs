...
use {
    ...
    itertools::Itertools,
    log::*,
    miraland_clap_utils::{hidden_unless_forced, input_validators::is_slot},
...
fn analyze_storage(database: &Database) -> Result<()> {
    use miraland_ledger::blockstore_db::columns::*;
...
fn raw_key_to_slot(key: &[u8], column_name: &str) -> Option<Slot> {
    use miraland_ledger::blockstore_db::columns as cf;
    match column_name {
...
pub fn blockstore_subcommands<'a, 'b>(hidden: bool) -> Vec<App<'a, 'b>> {
    ...
    vec![
        ...
        SubCommand::with_name("latest-optimistic-slots")
            ...
            // such that the subcommand will be visible as the top level of miraland-ledger-tool
            .arg(
