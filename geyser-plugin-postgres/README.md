The `miraland_geyser-plugin-postgres` crate implements a plugin storing
account data to a PostgreSQL database to illustrate how a plugin can be
developed to work with Miraland validators using the [Plugin Framework](https://docs.miraland.top/developing/plugins/geyser-plugins).
...
```
    "use_ssl": true,
    "server_ca": "/miraland/.ssh/server-ca.pem",
    "client_cert": "/miraland/.ssh/client-cert.pem",
    "client_key": "/miraland/.ssh/client-key.pem",
```
...
```
psql -U miraland -p 5433 -h 10.138.0.9 -w -d miraland -f scripts/create_schema.sql
```
...
```
psql -U miraland -p 5433 -h 10.138.0.9 -w -d miraland -f scripts/drop_schema.sql
```
