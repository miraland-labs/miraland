...
const RUST_LOG_FILTER: &str =
    "info,miraland_core::replay_stage=warn,miraland_local_cluster=info,local_cluster=info,miraland_ledger=info";
...
fn generate_geyser_plugin_config() -> (TempDir, PathBuf) {
    ...
    let lib_name = if std::env::consts::OS == "macos" {
        "libmiraland_geyser_plugin_postgres.dylib"
    } else {
        "libmiraland_geyser_plugin_postgres.so"
    };
...
fn test_postgres_plugin() {
    ...
    unsafe {
        let filename = match std::env::consts::OS {
            "macos" => "libmiraland_geyser_plugin_postgres.dylib",
            _ => "libmiraland_geyser_plugin_postgres.so",
        };
