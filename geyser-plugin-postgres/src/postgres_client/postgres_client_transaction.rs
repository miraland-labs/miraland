...
// MI: this worked for old version like 1.14.7
...
// // MI: 2023-05-07
// // in 1.18.0, in enum SanitizedMessage::Legacy(LegacyMessage)
// // LegacyMessage is redefined which is different with old version(1.14.7)
// // struct LegacyMessage is a wrapper of legacy::Message
// // add a From impl for geyser plugin postgres client, DbTransactionMessage::from()
// impl From<&LegacyMessage<'static>> for DbTransactionMessage {
//     fn from(message: &LegacyMessage<'static>) -> Self {
//         Self {
    ...
    DbTransaction {
        ...
        legacy_message: match transaction_info.transaction.message() {
            SanitizedMessage::Legacy(legacy_message) => {
                // Some(DbTransactionMessage::from(legacy_message)) // MI
