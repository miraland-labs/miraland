use {
    ...
    miraland_runtime::{bank::Bank, transaction_batch::TransactionBatch},
...
fn collect_token_balance_from_account(
    ...
    let decimals = mint_decimals
        .get(&Pubkey::from(mint.to_bytes()))
        .cloned()
        .or_else(|| {
            let decimals = get_mint_decimals(bank, &Pubkey::from(mint.to_bytes()))?;
            mint_decimals.insert(Pubkey::from(mint.to_bytes()), decimals);
            Some(decimals)
        })?;
